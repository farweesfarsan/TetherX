...
const BuyerDashboard = () => {
  ...
  const [sellerBreackdown, setSellerBreakdown] = useState([]);
  const [count,setCount] = useState(0);
  const [totalAcceptedAmount, setTotalAcceptedAmount] = useState(0);
...
useEffect(() => {
 ...
 useEffect(() => {
  const fetchBuyerPerformance = async () => {
    try {
      const token = localStorage.getItem('token');
               if(!token){
      ...
      const response = await axios.get('http://localhost:3001/getBuyerTransInfo',{
        headers:{ Authorization:`Bearer ${token}`}
      });
       console.log("Performance data",response.data);
      if (response.data.success) {
        setSellerBreakdown(response.data.sellerRecords);
        setCount(response.data.count);
        setTotalAcceptedAmount(response.data.acceptedAmount);
      }

      console.log("Buyer Breakdown",sellerBreackdown);
      console.log("total count",count);
      console.log("totla transaction amount",totalAcceptedAmount);
    } catch (error) {
...
{requests.length > 0 ? (
  requests.map((req, index) => (
    <div key={index} className='self-stretch rounded bg-[#183350] w-full flex flex-row items-start justify-start p-2 relative gap-2'>
       ...
       <p className='relative font-medium'>To: {req.user?.firstName} {req.user?.lastName}</p>
  ...
  <p>No requests found</p>
        ...
        <div className='uppercase text-[12px] absolute top-[420px] left-[270px] font-medium text-gray-400'>Recent Transaction</div>
             <div className='absolute top-[450px] left-[250px] grid grid-cols-1 sm:grid-cols-3 gap-2 p-2'>
                       <div className="bg-green-100 shadow-lg rounded-lg p-2 flex items-center justify-between">
                        <div>
                          <h2 className="relative top-[-20px] text-[17px] font-semibold text-gray-700">Total Purchased Amount</h2>
                          <p className="text-[15px] font-bold text-green-600">$ {totalAcceptedAmount}</p>
                          <p className="text-[13px] text-gray-500">+12% from last month</p>
                        </div>
                           <FcSalesPerformance className="text-green-500 text-8xl" />
                        </div>
             
                        <div className="bg-orange-100 shadow-lg rounded-lg p-2 flex items-center justify-between">
                        <div>
                          <h2 className="text-[17px] font-semibold text-gray-700">Count of Requests Sent</h2>
                          <p className=" text-[15px] font-bold  flex flex-row gap-2 bg-[#91f1a6] pl-2 rounded-md text-[#34a844]"><FaRegCheckCircle  className='relative mt-[4px] '/> - Accepted Requests- {count.accepted} </p>
                          <p className="text-[15px] font-bold flex flex-row text-orange-600 pl-2 bg-[#ecc28b] gap-2 rounded-md"><FaHourglassHalf className='relative mt-[4px]'/>- Pending Requests - {count.pending} </p>
                          <p className="text-[15px] font-bold flex flex-row text-[#e63f3f] pl-2 bg-[#e79292] gap-2 rounded-md">< GiCancel  className='relative mt-[4px]'/> - Rejected Requests - {count.rejected}</p>
                        </div>
                      </div>
                     
           
                    {sellerBreackdown.length > 0 ? (
                      <div>
                        {sellerBreackdown.map((seller) => (
                         <div key={seller.userId} className='bg-blue-100 shadow-lg rounded-lg w-[190px] h-[180px] p-2 flex items-center justify-between'>
                           <div>
                            <h2 className=" relative top-[-5px] text-[17px] font-semibold text-gray-700">Top Frequent Seller</h2>
                            <p className="text-[15px] font-bold text-blue-600">{seller.name}</p>
                            <p className="text-[15px] font-bold text-blue-600">Purchased Amount: ${seller.totalSpent}</p>
                           </div>
